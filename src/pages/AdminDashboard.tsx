...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    async function loadData() {
      if (isAdmin) {
        try {
          const [allUsers, allBookings, allTestimonials] = await Promise.all([
            getAllUsers(),
            getAllBookings(),
            getAllTestimonials(),
          ]);
  ...
  const handleToggleTestimonialPermission = async (userId: number) => {
    try {
      await toggleTestimonialPermission(userId);
      // Reload users
  ...
  const handleResetUserPassword = async (e: React.FormEvent) => {
    ...
    try {
      await updateUserPassword(resetUserId, resetPassword);
      toast({
